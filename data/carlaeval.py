...
def compute_statistics_jit(overlaps,
                           ...
                           compute_aos=False):
    '''
    Note: This function is kept without any change.
    Theoretically, it will give us the correct results,
    if we do not eval bbox2d and aos.
    '''
...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    '''
    Note: This function is changed greatly,
        since we do not want to filt MIN_HEIGHT,
        MAX_OCCLUSION, MAX_TRANCATION.
        We implement the MAX_DISTANCE for classify
    ...
    # Note: We comment out the next few lines,
    # since we do not want to filt MIN_HEIGHT,
    # MAX_OCCLUSION, MAX_TRANCATION.
    # We implement the MAX_DISTANCE for classify
    # the different difficulties.
    ...
    for i in range(num_gt):
        bcenter = gt_anno["location"][i]
        gt_name = gt_anno["name"][i].lower()
        x, y, z = bcenter.flatten()
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty):
    ...
    for i in range(len(gt_annos)):
        ...
        # Note: We changed the next few lines,
        # since we do not want to evaluate bbox2d.
        # We suspect the gt_datas & dt_datas have
        ...
        # ], 1)
        gt_datas = np.concatenate(
            [gt_annos[i]["location"], gt_annos[i]["alpha"][..., np.newaxis]], 1)
        dt_datas = np.concatenate([
            dt_annos[i]["location"], dt_annos[i]["alpha"][..., np.newaxis],
            dt_annos[i]["score"][..., np.newaxis]
        ], 1)
...
def do_eval_v3(gt_annos,
    ...
    types = ["bbox", "bev", "3d"]
    metrics = {}
    # Note: we change the next line,
    # since we do not need the bbox2d evaluation
    # for i in range(3):
    for i in range(1, 3):
        ret = eval_class_v3(
            gt_annos,
...
def get_eval_result(gt_annos,
    ...
    compute_aos = False
    # Note: we comment out the next few lines,
    # since we tempararilly do not need AOS evaluation
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            if compute_aos:
                assert False, "Currently we do not support comput_aos"
